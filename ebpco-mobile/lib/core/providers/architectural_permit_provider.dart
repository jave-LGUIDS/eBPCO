...
/// Holds the single in-progress Architectural Permit application draft for
/// the current app session (frontend-only: nothing here is persisted to
/// disk or a server). Mirrors the other four permit providers' shape
...
/// Addition/Extension, or Demolition draft.
class ArchitecturalPermitProvider extends ChangeNotifier {
  ArchitecturalPermitDraft? _draft;
  int _currentStep = 0;

  ArchitecturalPermitDraft? get draft => _draft;
  int get currentStep => _currentStep;

  /// Whether there is an unsubmitted draft the wizard can resume into.
  bool get hasResumableDraft =>
      _draft != null && _draft!.status == ArchitecturalPermitDraftStatus.draft;
  ...
  ArchitecturalPermitDraft resumeOrStart() {
  ...
  ArchitecturalPermitDraft startNew() {
  ...
  void submitApplication() {
    final draft = _draft;
    if (draft == null) return;
    draft.status = ArchitecturalPermitDraftStatus.submitted;
    notifyListeners();
  }
