...
/// persisted to disk or a server). Mirrors the other five permit
...
class CivilStructuralPermitProvider extends ChangeNotifier {
  ...
  CivilStructuralPermitDraft? get draft => _draft;
  int get currentStep => _currentStep;

  /// Whether there is an unsubmitted draft the wizard can resume into.
  bool get hasResumableDraft =>
      _draft != null && _draft!.status == CivilStructuralPermitDraftStatus.draft;
  ...
  CivilStructuralPermitDraft resumeOrStart() {
  ...
  CivilStructuralPermitDraft startNew() {
    final draft = CivilStructuralPermitDraft();
    _draft = draft;
    _currentStep = 0;
    return draft;
  }
  ...
  void submitApplication() {
    final draft = _draft;
    if (draft == null) return;
    draft.status = CivilStructuralPermitDraftStatus.submitted;
    notifyListeners();
  }
