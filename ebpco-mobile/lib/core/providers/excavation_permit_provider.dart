...
/// Holds the single in-progress Excavation & Ground Preparation Permit
...
/// permit's draft.
class ExcavationPermitProvider extends ChangeNotifier {
  ExcavationPermitDraft? _draft;
  int _currentStep = 0;

  ExcavationPermitDraft? get draft => _draft;
  int get currentStep => _currentStep;

  /// Whether there is an unsubmitted draft the wizard can resume into.
  bool get hasResumableDraft =>
      _draft != null && _draft!.status == ExcavationPermitDraftStatus.draft;
  ...
  ExcavationPermitDraft resumeOrStart() {
  ...
  ExcavationPermitDraft startNew() {
    final draft = ExcavationPermitDraft();
    _draft = draft;
    _currentStep = 0;
    return draft;
  }
  ...
  void submitApplication() {
    final draft = _draft;
    if (draft == null) return;
    draft.status = ExcavationPermitDraftStatus.submitted;
    notifyListeners();
  }
