...
class CertificateOfOccupancyProvider extends ChangeNotifier {
  ...
  CertificateOfOccupancyDraft? get draft => _draft;
  int get currentStep => _currentStep;

  /// Whether there is an unsubmitted draft the wizard can resume into.
  bool get hasResumableDraft =>
      _draft != null &&
      _draft!.status == CertificateOfOccupancyDraftStatus.draft;
  ...
  CertificateOfOccupancyDraft resumeOrStart() {
  ...
  CertificateOfOccupancyDraft startNew() {
  ...
  void submitApplication() {
    final draft = _draft;
    if (draft == null) return;
    draft.status = CertificateOfOccupancyDraftStatus.submitted;
    notifyListeners();
  }
