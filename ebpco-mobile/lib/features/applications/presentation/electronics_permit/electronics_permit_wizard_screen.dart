...
import 'steps/step4_installation_nature.dart';
...
/// Electronics Permit application wizard — a full 9-step flow based on
/// the official Electronics Permit form (Boxes 1–9), completely separate
/// from every other permit in this app: its own model
/// ([ElectronicsPermitDraft]), its own provider
/// ([ElectronicsPermitProvider]), and its own step widgets. Structurally
/// mirrors every other wizard (progress header, PageView, bottom action
/// bar) so all permits feel identical to use. See
/// `electronics_permit_model.dart` for how the official form's 10
/// numbered boxes map onto this app's established 9-step shape.
class ElectronicsPermitWizardScreen extends StatefulWidget {
  const ElectronicsPermitWizardScreen({super.key});

  @override
  State<ElectronicsPermitWizardScreen> createState() =>
      _ElectronicsPermitWizardScreenState();
}

class _ElectronicsPermitWizardScreenState
    extends State<ElectronicsPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle:
          'Provide the details of the person applying for the Electronics Permit.',
    ),
    _StepMeta(
      title: 'Address & Location of Construction',
      subtitle:
          'Provide the applicant address and location of the electronics work.',
    ),
    _StepMeta(
      title: 'Scope of Work',
      subtitle: 'Select the project scope covered by the Electronics Permit.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle: 'Upload the electronics documents required for evaluation.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle: 'Review the Electronics Permit application before submission.',
    ),
    _StepMeta(
  ...
  late ElectronicsPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<ElectronicsPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<ElectronicsPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    final referenceNumber =
        'ELX-${now.year}-${(now.millisecondsSinceEpoch % 900000 + 100000)}';
  ...
  void _handleSaveDraft() {
    context.read<ElectronicsPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Electronics Permit'),
          leading: IconButton(
