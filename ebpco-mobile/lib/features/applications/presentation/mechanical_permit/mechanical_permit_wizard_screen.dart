...
/// Mechanical Permit application wizard — a full 9-step flow based on the
/// official Mechanical Permit form, completely separate from New
/// Construction, Renovation, Addition/Extension, Demolition,
/// Architectural, Civil/Structural, and Electrical: its own model
/// ([MechanicalPermitDraft]), its own provider
/// ([MechanicalPermitProvider]), and its own step widgets. Structurally
/// mirrors the other seven wizards (progress header, PageView, bottom
/// action bar) so all eight feel identical to use.
class MechanicalPermitWizardScreen extends StatefulWidget {
  const MechanicalPermitWizardScreen({super.key});

  @override
  State<MechanicalPermitWizardScreen> createState() =>
      _MechanicalPermitWizardScreenState();
}

class _MechanicalPermitWizardScreenState
    extends State<MechanicalPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    ...
    _StepMeta(
      ...
      subtitle: 'Select the project scope covered by the Mechanical Permit.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle: 'Upload the mechanical documents required for evaluation.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle: 'Review the Mechanical Permit application before submission.',
    ),
    _StepMeta(
  ...
  late MechanicalPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<MechanicalPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<MechanicalPermitProvider>().goToStep(step);
  ...
  void _handleSaveDraft() {
    context.read<MechanicalPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Mechanical Permit'),
          leading: IconButton(
