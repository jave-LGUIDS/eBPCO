...
/// Step 5 — Civil / Structural Professionals: the Design Engineer and the
/// Supervisor / Engineer in Charge. When the same person fills both
/// roles, the Supervisor's own fields/uploads are never populated — a
/// read-only summary is shown instead, and Step 7 reads the Design
/// Engineer's uploads for both roles in that case.
...
class _Step5ProfessionalsState extends State<Step5Professionals> {
  ...
  void initState() {
    super.initState();
    final design = _professionals.designEngineer;
        ...
        child: Column(
          ...
          children: [
            const AppAlert(
              ...
              message:
                  'The Civil / Structural Permit requires a licensed Civil '
                  'or Structural Engineer to sign and seal the plans, and a '
                  'licensed engineer to supervise the work.',
            ),

            const SizedBox(height: AppSpacing.xl),
            Text('Design Civil / Structural Engineer', style: AppTypography.cardTitle),
            ...
            AppCard(
              child: Column(
                ...
                children: [
                  ...
                  AppDropdown<CivilStructuralProfessionType>(
                    value: design.profession,
                    label: 'Profession *',
                    items: CivilStructuralProfessionType.values
                        .map(
                          (p) => DropdownMenuItem(
                            value: p,
                            child: Text(p.label),
            ...
            const SizedBox(height: AppSpacing.md),
            DocumentUploadTile(
              label: 'Signed and Sealed Design Computations',
              document: _professionals.signedSealedComputationsUpload,
              onUpload: () {
                setState(() {
                  _professionals.signedSealedComputationsUpload =
                      createMockDocument('Signed and Sealed Design Computations');
                });
                widget.onChanged();
              },
              allowReplace: true,
              onRemove: () {
                setState(
                  () => _professionals.signedSealedComputationsUpload = null,
                );
                widget.onChanged();
              },
            ),
            ...
            AppCard(
              child: SwitchListTile.adaptive(
                ...
                title: const Text(
                  'Is the Design Engineer also the Supervisor / In-Charge?',
                ),
                value: _professionals.isSupervisorSameAsDesignEngineer,
                onChanged: (v) {
                  setState(() => _professionals.isSupervisorSameAsDesignEngineer = v);
                  widget.onChanged();
                },
              ),
            ),
            if (_professionals.isSupervisorSameAsDesignEngineer) ...[
              const SizedBox(height: AppSpacing.md),
              AppCard(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    Text('Same as Design Engineer', style: AppTypography.bodyStrong),
                    const SizedBox(height: AppSpacing.sm),
