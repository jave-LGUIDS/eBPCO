...
import 'steps/step4_safety_site_prep.dart';
import 'steps/step5_demolition_supervisor.dart';
...
/// Demolition Permit application wizard — a full 9-step flow based on the
/// dedicated official Demolition Permit form, completely separate from
/// New Construction, Renovation, and Addition/Extension: its own model
/// ([DemolitionPermitDraft]), its own provider ([DemolitionPermitProvider]),
/// and its own step widgets. Structurally mirrors the other three wizards
/// (progress header, PageView, bottom action bar) so all four feel
/// identical to use.
class DemolitionPermitWizardScreen extends StatefulWidget {
  const DemolitionPermitWizardScreen({super.key});

  @override
  State<DemolitionPermitWizardScreen> createState() =>
      _DemolitionPermitWizardScreenState();
}

class _DemolitionPermitWizardScreenState
    extends State<DemolitionPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle: 'Tell us who is applying for the demolition permit.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle: 'Describe the structure and the scope of demolition work.',
    ),
    _StepMeta(
      title: 'Safety & Site Preparation',
      subtitle:
          'Confirm occupancy, utility disconnection, and site safety measures.',
    ),
    _StepMeta(
      title: 'Demolition Supervisor',
      subtitle:
          'Provide the licensed professional supervising the demolition work.',
    ),
    _StepMeta(
      title: 'Ownership & Consent',
      subtitle: 'Confirm your authority to demolish the structure.',
    ),
    _StepMeta(
      title: 'Required Documents',
      subtitle:
          'Upload the documents needed to evaluate the demolition application.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle: 'Review your demolition application before submission.',
    ),
    _StepMeta(
      title: 'Evaluation & Permit Status',
      subtitle: 'Review the evaluation status of your demolition application.',
    ),
  ];
  ...
  late DemolitionPermitDraft _draft;
  ...
  void _goToStep(int step) {
    ...
    context.read<DemolitionPermitProvider>().goToStep(step);
  ...
  void _handleSaveDraft() {
    context.read<DemolitionPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Demolition Permit'),
          leading: IconButton(
