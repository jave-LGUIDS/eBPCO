...
import 'steps/step4_building_addition_details.dart';
...
/// Addition / Extension Permit application wizard — a full 9-step flow
/// completely separate from the New Construction and Renovation wizards:
/// its own model ([AdditionExtensionPermitDraft]), its own provider
/// ([AdditionExtensionPermitProvider]), and its own step widgets, so
/// nothing here can ever read or overwrite either of the other two
/// drafts. Structurally mirrors [BuildingPermitWizardScreen] and
/// [RenovationPermitWizardScreen] (progress header, PageView, bottom
/// action bar) so all three wizards feel identical to use.
class AdditionExtensionPermitWizardScreen extends StatefulWidget {
  const AdditionExtensionPermitWizardScreen({super.key});

  @override
  State<AdditionExtensionPermitWizardScreen> createState() =>
      _AdditionExtensionPermitWizardScreenState();
}

class _AdditionExtensionPermitWizardScreenState
    extends State<AdditionExtensionPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    ...
    _StepMeta(
      ...
      subtitle: 'Describe what will be added to the existing building.',
    ),
    _StepMeta(
      title: 'Building & Addition Details',
      subtitle:
          'Provide measurements for the existing building and the proposed added area.',
    ),
    _StepMeta(
      title: 'Professional in Charge',
      subtitle:
          'Provide the licensed professional supervising the Addition / Extension work.',
    ),
    _StepMeta(
      title: 'Ownership & Authorization',
      subtitle: 'Confirm your authority to add to or extend the property.',
    ),
    _StepMeta(
      title: 'Required Documents',
      subtitle:
          'Upload the documents needed to evaluate the Addition / Extension application.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle:
          'Review the assessment status of the Addition / Extension application.',
    ),
  ];
  ...
  late AdditionExtensionPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<AdditionExtensionPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<AdditionExtensionPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    final referenceNumber =
        'ADX-${now.year}-${(now.millisecondsSinceEpoch % 900000 + 100000)}';
  ...
  void _handleSaveDraft() {
    context.read<AdditionExtensionPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Addition / Extension Permit'),
          leading: IconButton(
