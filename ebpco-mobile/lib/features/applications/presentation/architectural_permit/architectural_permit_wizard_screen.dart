...
import 'steps/step4_compliance_details.dart';
...
/// Architectural Permit application wizard — a full 9-step flow based on
/// the official Architectural Permit form (Boxes 1-9), completely
/// separate from New Construction, Renovation, Addition/Extension, and
/// Demolition: its own model ([ArchitecturalPermitDraft]), its own
/// provider ([ArchitecturalPermitProvider]), and its own step widgets.
/// Structurally mirrors the other four wizards (progress header,
/// PageView, bottom action bar) so all five feel identical to use.
class ArchitecturalPermitWizardScreen extends StatefulWidget {
  const ArchitecturalPermitWizardScreen({super.key});

  @override
  State<ArchitecturalPermitWizardScreen> createState() =>
      _ArchitecturalPermitWizardScreenState();
}

class _ArchitecturalPermitWizardScreenState
    extends State<ArchitecturalPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    ...
    _StepMeta(
      ...
      subtitle: 'Select the architectural work included in this permit.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle: 'Upload the architectural documents required for evaluation.',
    ),
    _StepMeta(
  ...
  late ArchitecturalPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<ArchitecturalPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<ArchitecturalPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    final referenceNumber =
        'ARC-${now.year}-${(now.millisecondsSinceEpoch % 900000 + 100000)}';
  ...
  void _handleSaveDraft() {
    context.read<ArchitecturalPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Architectural Permit'),
          leading: IconButton(
