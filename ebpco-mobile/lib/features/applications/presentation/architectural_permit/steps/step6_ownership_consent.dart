...
/// Step 6 — Ownership & Consent (Box 5 + Box 6): confirms whether the
/// applicant is the Building Owner, and separately whether the Building
/// Owner is also the registered Lot Owner. Hidden fields never block
/// navigation, and switching an answer back and forth preserves whatever
/// was already entered.
...
class _Step6OwnershipConsentState extends State<Step6OwnershipConsent> {
  ...
  ArchitecturalOwnershipConsent get _consent => widget.draft.ownershipConsent;

  @override
  void initState() {
    super.initState();
    final buildingOwner = _consent.buildingOwner;
    _buildingOwnerFullName = TextEditingController(text: buildingOwner.fullName);
    _buildingOwnerAddress = TextEditingController(text: buildingOwner.address);
    _buildingOwnerCtcNumber = TextEditingController(text: buildingOwner.ctcNumber);
    ...
    _lotOwnerCtcPlaceIssued = TextEditingController(text: lotOwner.ctcPlaceIssued);
  }
