...
import 'steps/step4_nature_of_work.dart';
...
/// Interior Design Permit application wizard — a full 9-step flow based
/// on the official two-page Interior Design Permit form, completely
/// separate from every other permit in this app: its own model
/// ([InteriorPermitDraft]), its own provider
/// ([InteriorDesignPermitProvider]), and its own step widgets.
/// Structurally mirrors every other wizard (progress header, PageView,
/// bottom action bar) so all permits feel identical to use. See
/// `interior_design_permit_model.dart` for how the official form's
/// sections map onto this app's established 9-step shape.
class InteriorDesignPermitWizardScreen extends StatefulWidget {
  const InteriorDesignPermitWizardScreen({super.key});

  @override
  State<InteriorDesignPermitWizardScreen> createState() =>
      _InteriorDesignPermitWizardScreenState();
}

class _InteriorDesignPermitWizardScreenState
    extends State<InteriorDesignPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle:
          'Provide the details of the person applying for the Interior Design Permit.',
    ),
    _StepMeta(
      title: 'Address & Location of Construction',
      subtitle:
          'Provide the applicant address and location of the interior works.',
    ),
    _StepMeta(
      title: 'Scope of Work',
      subtitle:
          'Select the project scope covered by the Interior Design Permit.',
    ),
    _StepMeta(
      title: 'Nature of Interior Works',
      subtitle: 'Select the type of interior work involved in the project.',
    ),
    _StepMeta(
      title: 'Interior Design Professionals',
      subtitle:
    ...
    _StepMeta(
      ...
      subtitle: 'Upload the interior design documents required for evaluation.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle:
          'Review the Interior Design Permit application before submission.',
    ),
    _StepMeta(
  ...
  late InteriorPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<InteriorDesignPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<InteriorDesignPermitProvider>().goToStep(step);
  ...
  void _handleSaveDraft() {
    context.read<InteriorDesignPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Interior Design Permit'),
          leading: IconButton(
