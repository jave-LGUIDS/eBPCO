...
/// Plumbing Permit application wizard — a full 9-step flow based on the
/// official Plumbing Permit form, completely separate from New
/// Construction, Renovation, Addition/Extension, Demolition,
/// Architectural, Civil/Structural, Electrical, Mechanical, and — most
/// importantly — the combined Sanitary / Plumbing Permit: its own model
/// ([PlumbingPermitDraft]), its own provider ([PlumbingPermitProvider]),
/// and its own step widgets. Structurally mirrors the other nine wizards
/// (progress header, PageView, bottom action bar) so all ten feel
/// identical to use.
class PlumbingPermitWizardScreen extends StatefulWidget {
  const PlumbingPermitWizardScreen({super.key});

  @override
  State<PlumbingPermitWizardScreen> createState() =>
      _PlumbingPermitWizardScreenState();
}

class _PlumbingPermitWizardScreenState
    extends State<PlumbingPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle:
          'Provide the details of the person applying for the Plumbing Permit.',
    ),
    _StepMeta(
      title: 'Address & Project Location',
      subtitle:
          'Provide the applicant address and location of the plumbing work.',
    ),
    _StepMeta(
      title: 'Scope of Plumbing Work',
      subtitle: 'Select the project scope covered by the Plumbing Permit.',
    ),
    _StepMeta(
      title: 'Plumbing Installation Details',
      subtitle:
          'Provide fixture quantities and identify the plumbing systems included in the project.',
    ),
    _StepMeta(
      title: 'Plumbing Professionals',
      subtitle:
          'Provide the Master Plumbers responsible for design and supervision.',
    ),
    _StepMeta(
      title: 'Ownership & Consent',
      subtitle:
          'Confirm the Building Owner and property-authorization information.',
    ),
    _StepMeta(
      title: 'Required Documents',
      subtitle: 'Upload the plumbing documents required for evaluation.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle: 'Review the Plumbing Permit application before submission.',
    ),
    _StepMeta(
  ...
  late PlumbingPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<PlumbingPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<PlumbingPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    final referenceNumber =
        'PLB-${now.year}-${(now.millisecondsSinceEpoch % 900000 + 100000)}';
  ...
  void _handleSaveDraft() {
    context.read<PlumbingPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Plumbing Permit'),
          leading: IconButton(
