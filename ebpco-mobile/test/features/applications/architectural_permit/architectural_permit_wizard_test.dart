...
/// End-to-end coverage of the Architectural Permit wizard — fully separate
/// from the New Construction, Renovation, Addition/Extension, and
/// Demolition wizards, driven the same way those wizards' tests drive them.
Widget _wrap() {
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return ArchitecturalApplicationSubmittedScreen(
            referenceNumber: extra?['referenceNumber'] as String? ?? 'ARC-X',
            submissionDate:
...
Future<void> _useTallSurface(WidgetTester tester) async {
  await tester.binding.setSurfaceSize(const Size(400, 4800));
...
/// Leaves the Related Building Permit at its default "Pending Issuance"
/// status, which is a valid state (no Building Permit Number required).
...
Future<void> _completeStep3(WidgetTester tester) async {
  ...
  await tester.pump();
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Architectural Work Title *'),
    'New Two-Storey Residence',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Description of Architectural Work *'),
    'Construction of a new two-storey residential building.',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Building Areas Affected *'),
    'Entire building.',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Existing Condition *'),
    'Vacant lot.',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Proposed Architectural Changes *'),
    'New two-storey residential structure.',
  );
  await tester.pump();
  await tester.tap(_continueButton());
  await tester.pumpAndSettle();
...
/// Leaves every accessibility facility and Fire Code feature at "Not
/// Applicable" so no conditional Step 7 documents are triggered.
Future<void> _completeStep4(WidgetTester tester) async {
  ...
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Unpaved Surface Area Percentage (%) *'),
    '40',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Number of Exit Doors *'),
    '2',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Total Exit Width (m) *'),
    '2.4',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Minimum Corridor Width (m) *'),
    '1.2',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Maximum Distance to Fire Exit (m) *'),
    '30',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Public Street Access Description *'),
    'Directly accessible from the main road.',
  );
  await tester.enterText(
    find.widgetWithText(TextFormField, 'Fire Wall Description *'),
...
Future<void> _completeStep7(WidgetTester tester) async {
  // "Location and Site Documents" starts expanded already, so it's
  // deliberately excluded here — tapping it again would collapse it.
  for (final section in [
    'Architectural Presentation',
    'Accessibility and Detail Drawings',
...
Future<void> _checkAllDeclarations(WidgetTester tester) async {
  for (final label in [
    ...
    'I understand that this Architectural Permit must be accompanied by a valid Building Permit.',
    'I understand that work must follow the approved plans and specifications.',
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.textContaining(
          'This permit cannot be valid or issued until your related Building Permit is issued.',
        ),
        findsOneWidget,
      );
    },
  ...
  testWidgets(
    'Selecting "Issued" status requires a Building Permit Number in Step 2',
    (tester) async {
      await _useTallSurface(tester);
      await _openWizard(tester);
      await _completeStep1(tester);
      ...
      expect(
        tester.widget<ElevatedButton>(_continueButton()).onPressed,
        isNotNull,
        reason: 'Pending Issuance does not require a Building Permit number',
      );
      ...
      expect(
        tester.widget<ElevatedButton>(_continueButton()).onPressed,
        isNull,
        reason: 'Issued status requires a Building Permit number',
      );
  ...
  testWidgets(
    'Selecting Ramps accessibility makes Ramp Details conditionally required in Step 7',
    (tester) async {
      await _useTallSurface(tester);
      await _openWizard(tester);
      await _completeStep1(tester);
      ...
      final rampsDropdown = find.ancestor(
        of: find.text('Ramps'),
        matching: find.byWidgetPredicate((w) => w is DropdownButtonFormField),
      );
      await tester.ensureVisible(rampsDropdown);
      await tester.pumpAndSettle();
      await tester.tap(rampsDropdown);
      await tester.pumpAndSettle();
      await tester.tap(find.text('Proposed').last);
      await tester.pumpAndSettle();
      ...
      await tester.enterText(
        find.widgetWithText(
          TextFormField,
          'Maximum Distance to Fire Exit (m) *',
        ),
        '30',
      );
      await tester.enterText(
        find.widgetWithText(
          TextFormField,
          'Public Street Access Description *',
        ),
        'Directly accessible from the main road.',
      );
      await tester.enterText(
      ...
      expect(find.text('Step 7 of 9'), findsOneWidget);
      await tester.ensureVisible(
        find.text('Accessibility and Detail Drawings'),
      );
      await tester.pumpAndSettle();
      await tester.tap(find.text('Accessibility and Detail Drawings'));
      await tester.pumpAndSettle();

      expect(
        find.textContaining('Conditionally required — Ramps selected'),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets('Back navigation preserves data across Steps 1-3', (
    ...
    expect(find.text('Step 3 of 9'), findsOneWidget);
    expect(
      find.widgetWithText(TextFormField, 'New Two-Storey Residence'),
      findsOneWidget,
    );
